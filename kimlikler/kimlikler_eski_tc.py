...
def soyad_label_mi(text):
    ...
    if not norm:
        return False
    if "SOYAD" in norm or "SOYADI" in norm:
        return True
...
def herhangi_bir_label_mi(text):
    ...
    if not norm:
        return False
    if tc_label_mi(text) or soyad_label_mi(text) or ad_label_mi(text):
        return True
...
def ayni_satir_mi(item1, item2, minimum_tolerans=20):
...
def label_satirindaki_deger_bul(ocr, label):
    if label is None:
        return None
...
def eski_tc_no_bul(ocr):
    ...
    tc_label = en_iyi_label_bul(ocr, "tc")
    adaylar = []
...
def tc_altindaki_deger_satirlari(ocr, tc_item):
    ...
    adaylar = [
        ...
        if isim_value_adayi_mi(item) and merkez_y(item) > tc_y and yukseklik(item) >= 16
    ]
...
def eski_tc_bilgilerini_bul(ocr_sonuclari):
    ...
    soyad_sonuc = label_satirindaki_deger_bul(ocr_sonuclari, soyad_label)
    ad_sonuc = label_satirindaki_deger_bul(ocr_sonuclari, ad_label)
    ...
    tum_satirlar = tc_altindaki_deger_satirlari(ocr_sonuclari, tc_item)
